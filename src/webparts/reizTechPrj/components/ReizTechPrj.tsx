...
import {
  ...
  YEAR_DAY_COUNT,
  DATE_TO_DAY,
  TIME_CONVERSION_DATA,
} from "../common/constant";

import { sp } from "@pnp/sp";

export default class ReizTechPrj extends React.Component<
  IReizTechPrjProps,
  IReactPartialStateUpdateState
> {
  public constructor(
    props: IReizTechPrjProps,
    state: IReactPartialStateUpdateState
  ) {
    super(props);
      ...
      <section>
        ...
        {this.state.showWelcome === true && (
          <div className="container-homepage">
            ...
            <p>
              Thank you for visiting our website. Please read the instruction
              carefully and follow the instructions as mentioned below:
            </p>
            <ul>
              <li>Attempt all questions</li>
              <li>
                You will be able to get your result after submitting your
                response
              </li>
              <li>
                After clicking continue , you will not be able to come back to
                this screen
              </li>
              <li>
                There will be no negative marking, so attempt all questions
              </li>
            </ul>
            <h3>Note: </h3>
            <p>
              We hope you have a pleasant experience and in case of any query
              feel free to call Hadi Mahmood
            </p>
            <a className="button" onClick={() => this.moveToQuiz()}>
              Continue
            </a>
          </div>
        )}
            ...
            <div className="container3">
              ...
              <form action="#" method="post">
                ...
                <div className="container2">
                  <h3>
                    Please choose a correct answer from the given options:
                  </h3>
                  <ol>
                    <li>
                      <label>
                        If 2021-03-14 is on Sunday what will be the day of the
                        month considering same date, month but year 2025, i.e.
                        the day on 2025-03-14?
                      </label>
                      <br />
                      <select
                        id="DayYearChange"
                        value={this.state.selectedDay_DayYearChange}
                        onChange={this.handleSelectionChangeDayYearChange}
                      >
                    ...
                    <li>
                      <label>
                        If Amsterdam time zone is 16 hours ahead from LosAngles
                        timezone.Calculate time at Amsterdam when it is 06:30:00
                        am in LosAngeles
                      </label>
                    </li>
                    <br />
                    ...
                    <select
                      id="AmsterdamLosAngles"
                      value={this.state.selected_AmsterdamLosAngles}
                      onChange={this.handleTimeZoneConversion}
                    >
                ...
                <div className="container2">
                  ...
                  <ol>
                    <li>
                      <label>
                        How many days have passed before your birthday in a
                        year? Provided your birthday is
                      </label>
                      <input
                        type="date"
                        id="beforeBirthdate"
                        onChange={this.handleBeforeBdDate}
                        value={this.state.selected_before_bd}
                        className="blank-style"
                      ...
                      <input
                        type="number"
                        onChange={this.handleBeforeBdDays}
                        value={this.state.before_birthdays}
                        className="blank-style"
                        id="Days"
                    ...
                    <li>
                      <label>
                        How many days are left in a year after your birthday ?
                        Provided your birthday is{" "}
                      </label>
                      <input
                        type="date"
                        onChange={this.handleAfterBdDate}
                        value={this.state.selected_after_bd}
                        className="blank-style"
                        id="afterBirthdate"
                        placeholder="Birthdate"
                        name="birthdate"
                    ...
                    <input
                      type="number"
                      onChange={this.handleAfterBdDays}
                      value={this.state.after_birthdays}
                      className="blank-style"
                      placeholder="Enter your Answer Day "
                      id="DayForBirth"
                      name="Day for Birth"
                      required
                    />
                ...
                {/* <!-- Submit Button --> */}
                <b>Enter All Values to see the Submit Button</b>
                <br />
        ...
        {this.state.showEnd === true && (
          <div className="container-end">
            ...
            <a onClick={() => this.moveToHome()} className="button">
              Return to Home{" "}
            </a>
          </div>
        )}
  ...
  handleSelectionChange_DayYearChange = (
  ...
  checkItemDataEntered = () => {
    if (
      this.state.user_name.toString().length > 0 &&
  ...
  async moveToEnd() {
    ...
    await this.countDaysBeforeBirthday(
      this.state.selectedBeforeBd,
      this.state.beforeBirthdays
    );
    await this.countDaysAfterBirthday(
      this.state.selectedAfterBd,
      this.state.afterBirthdays
    );
  ...
  private convertTimeZone = async (selectedDay: string) => {
    ...
    try {
      const responseData = await timeProxyCallWithBody(
        fullUrl,
        TIME_CONVERSION_DATA
      );
      if (responseData.time == selectedDay) {
        ...
        this.setState({
          userResponses: `${this.state.userResponses} Q1: ${selectedDay}`,
        });
        this.setState({
          expectedResponse: `${this.state.expectedResponse} A1: ${responseData.time} `,
        });
      } else {
  ...
  private fetchDayOnDate = async (selectedDay: any) => {
    ...
    try {
      ...
      if (data.dayOfWeek == selectedDay) {
        ...
        this.setState({
          expectedResponse: `${this.state.expectedResponse} A1: ${data.dayOfWeek} `,
        });
      } else {
        this.setState({ wrongAnswers: this.state.wrongAnswers + 1 });
        this.setState({
  ...
  private countDaysBeforeBirthday = async (birthday: any, selectedDay: any) => {
    ...
    try {
      ...
      if (parseInt(data.day) === parseInt(selectedDay)) {
        this.setState({ correctAnswers: this.state.correctAnswers + 1 });
        this.setState({
          userResponses: `${this.state.userResponses} Q3: ${selectedDay} with SelectedDate ${birthday}`,
        });
        this.setState({
          expectedResponse: `${this.state.expectedResponse} A3: ${parseInt(
            data.day
          )} with selectedDate ${birthday}`,
        });
      } else {
        this.setState({ wrongAnswers: this.state.wrongAnswers + 1 });
        this.setState({
  ...
  private countDaysAfterBirthday = async (birthday: any, selectedDay: any) => {
    ...
    try {
      ...
      if (YEAR_DAY_COUNT - parseInt(data.day) == parseInt(selectedDay)) {
        this.setState({ correctAnswers: this.state.correctAnswers + 1 });
        this.setState({
          userResponses: `${this.state.userResponses} Q4: ${selectedDay} with SelectedDate ${birthday}`,
        });
        this.setState({
          expectedResponse: `${this.state.expectedResponse} A4: ${parseInt(
            data.day
          )} with selectedDate ${birthday}`,
        });
      } else {
